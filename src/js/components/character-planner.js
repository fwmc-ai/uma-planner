    ...
    function createCharacterPlanner() {
        ...
        const getTrainingRecommendations = () => {
            ...
            const recommendations = trainingOptions.map(training => {
                ...
                // Calculate support card multipliers for this training type
                const selectedCards = (appState.selectedSupportCards || []).filter(cardData => cardData && cardData.card);
                const trainingMultipliers = window.Uma?.SupportCards?.calculateTrainingMultipliers ? 
                    window.Uma.SupportCards.calculateTrainingMultipliers(selectedCards, training.type, true) : 
                    { totalMultiplier: 1.0, friendshipBonus: 1.0, moodMultiplier: 1.0, trainingEffectiveness: 1.0, typeSpecificBonus: 1.0 };
                
                // Apply mood multiplier and support card bonuses to training gains
                const baseMoodMultiplier = window.Uma?.Constants?.MOOD_MULTIPLIERS?.[appState.currentMood]?.value || 1.0;
                const combinedMultiplier = baseMoodMultiplier * trainingMultipliers.totalMultiplier;
                const moodAdjustedPrimaryGain = Math.round(training.primaryGain * combinedMultiplier);
                
                // Calculate total value of this training
                let totalValue = 0;
                let statGains = {};
                
                // Primary stat value (with mood adjustment)
                if (primaryDeficit > 0) {
                    const primaryProgress = Math.min(moodAdjustedPrimaryGain, primaryDeficit);
                    totalValue += primaryProgress * 2; // Primary stats weighted higher
                    statGains[training.primaryStat] = moodAdjustedPrimaryGain;
                }
                
                // Secondary stat values (with mood and support card adjustment)
                Object.entries(training.secondaryStats).forEach(([stat, gain]) => {
                    ...
                    const moodAdjustedGain = Math.round(gain * combinedMultiplier);
                    
                    if (deficit > 0) {
                        const progress = Math.min(moodAdjustedGain, deficit);
                        totalValue += progress * 1; // Secondary stats normal weight
                        statGains[stat] = moodAdjustedGain;
                    }
                });
                ...
                return {
                    ...
                    trainingMultipliers,
                    combinedMultiplier,
                    recommendation: generateTrainingAdvice(training, primaryDeficit, statGains, specialNotes, trainingMultipliers)
                };
            });
        ...
        const generateTrainingAdvice = (training, deficit, statGains, specialNotes, trainingMultipliers) => {
            ...
            // Add support card bonus information
            if (trainingMultipliers && trainingMultipliers.totalMultiplier > 1.0) {
                const bonusPercent = Math.round((trainingMultipliers.totalMultiplier - 1.0) * 100);
                advice += ` (${bonusPercent}% bonus from support cards)`;
            }
        ...
        const getOverallStatus = () => {
            ...
            // Check for critical efficiency warnings only (exclude wit/guts since they have local warnings)
            const criticalEfficiencyWarnings = Object.entries(appState.stats)
              .map(([stat, value]) => {
                // Only show speed and stamina overflow warnings at the top level
                if (stat === 'speed' && value > 1200) {
                  return getStatEfficiencyWarning(stat, value, thresholds[stat], appState.targetTrack);
                }
                if (stat === 'stamina') {
                  const excessiveThresholds = {
                    sprint: 400, mile: 500, medium: 550, long: 650
                  };
                  if (value > excessiveThresholds[appState.targetTrack] + 200) {
                    return getStatEfficiencyWarning(stat, value, thresholds[stat], appState.targetTrack);
                  }
                }
                return null;
            ...
            if (criticalEfficiencyWarnings.length > 0) {
              return { 
                status: 'warning', 
                message: `⚠️ Efficiency Warning: ${criticalEfficiencyWarnings[0]}` 
              };
            }
        ...
        // Phase 4A Enhancement: Use enhanced training recommendations with mood integration
        const enhancedAdvice = window.Uma?.CharacterAdvice?.getEnhancedTrainingRecommendations 
            ? window.Uma.CharacterAdvice.getEnhancedTrainingRecommendations(character, appState.stats, appState.targetTrack, appState.style, appState.currentMood)
            : null;
        
        // Fallback to basic advice if enhanced system not available
        const characterAdvice = enhancedAdvice || getCharacterTrainingAdvice(character, appState.targetTrack, appState.style);
        ...
        leftPanel.innerHTML = `
            ...
            <!-- Phase 4A Enhancement: Mood Multiplier Section -->
            <div class="bg-purple-50 border border-purple-200 rounded-lg p-4 space-y-3">
                ...
                <select id="mood-select" class="w-full px-3 py-2 border border-purple-300 rounded-md focus:ring-2 focus:ring-purple-500 focus:border-transparent">
                ...
                <div id="mood-feedback" class="text-sm p-2 rounded-md bg-purple-100 text-purple-800">
                    💡 Normal mood - baseline training efficiency
                </div>
            </div>

            <!-- Phase 4B Enhancement: Support Card Selection -->
            <div class="bg-yellow-50 border border-yellow-200 rounded-lg p-4 space-y-3">
                <h3 class="font-semibold text-yellow-800 flex items-center gap-2">
                    <span>🎴</span>
                    Support Cards
                    <span class="text-xs bg-yellow-100 px-2 py-1 rounded-full text-yellow-600 font-normal">
                        ${(appState.selectedSupportCards || []).length}/6
                    </span>
                </h3>
                <div class="text-xs text-yellow-700 mb-2">
                    Support cards provide stat bonuses and special effects during training. Click to select up to 6 cards.
                </div>
                
                <!-- Support Card Slots (1-6) -->
                <div class="grid grid-cols-6 gap-1 mb-3">
                    ${Array.from({length: 6}, (_, i) => {
                        const cardId = (appState.selectedSupportCards || [])[i];
                        const card = cardId ? window.Uma?.SupportCards?.getSupportCardById(cardId) : null;
                        return `
                            <div class="aspect-square bg-white border border-yellow-300 rounded text-xs flex flex-col items-center justify-center p-1">
                                <div class="text-yellow-700 font-bold">${i + 1}</div>
                                ${card ? `
                                    <div class="text-center">
                                        <div class="font-medium truncate w-full">${card.name}</div>
                                        <div class="text-xs text-${card.type === 'Speed' ? 'red' : card.type === 'Power' ? 'blue' : 'purple'}-600">${card.rarity}</div>
                                    </div>
                ...
                <div class="space-y-2">
                    <div>
                        <label class="block text-sm font-medium text-yellow-800 mb-1">Available Cards</label>
                        <div class="grid grid-cols-1 gap-1 max-h-32 overflow-y-auto">
                            ${[...window.Uma?.SupportCards?.getSupportCardsByType('Speed') || [], 
                               ...window.Uma?.SupportCards?.getSupportCardsByType('Power') || [],
                               ...window.Uma?.SupportCards?.getSupportCardsByType('Stamina') || [],
                               ...window.Uma?.SupportCards?.getSupportCardsByType('Guts') || [],
                               ...window.Uma?.SupportCards?.getSupportCardsByType('Wit') || []].map(card => {
                                   const isSelected = (appState.selectedSupportCards || []).some(sc => sc.cardId === card.id);
                                   const selectedCard = (appState.selectedSupportCards || []).find(sc => sc.cardId === card.id);
                                   const currentLevel = selectedCard?.level || 1;
                                   const availableLevels = window.Uma?.SupportCards?.getAvailableLevels(card) || [1];
                                   
                                   return `
                                <div class="flex items-center space-x-2 p-2 bg-white rounded text-xs border hover:bg-gray-50 ${isSelected ? 'ring-2 ring-pink-300' : ''}">
                                    <input type="checkbox" 
                                           id="card-${card.id}" 
                                           class="support-card-checkbox" 
                                           data-card-id="${card.id}"
                                           ${isSelected ? 'checked' : ''}>
                                    <label for="card-${card.id}" class="flex-1 cursor-pointer">
                                        <div class="flex items-center justify-between mb-1">
                                            <span class="font-medium">${card.name}</span>
                                            <span class="text-${card.type === 'Speed' ? 'red' : card.type === 'Power' ? 'blue' : card.type === 'Stamina' ? 'green' : card.type === 'Guts' ? 'orange' : 'purple'}-600">${card.rarity} ${card.type}</span>
                                        </div>
                                        ${isSelected ? `
                                        <div class="flex items-center gap-2 mt-1">
                                            <span class="text-xs text-gray-600">Level:</span>
                                            <select class="support-card-level text-xs border rounded px-1 py-0.5" 
                                                    data-card-id="${card.id}" 
                                            ...
                                            <select class="support-card-bond text-xs border rounded px-1 py-0.5" 
                                                    data-card-id="${card.id}" 
                                                    onclick="event.stopPropagation()">
                                                <option value="unbonded" ${(selectedCard?.bondStatus || 'unbonded') === 'unbonded' ? 'selected' : ''}>Unbonded</option>
                                                <option value="bonded" ${(selectedCard?.bondStatus || 'unbonded') === 'bonded' ? 'selected' : ''}>Bonded</option>
                                                <option value="orange" ${(selectedCard?.bondStatus || 'unbonded') === 'orange' ? 'selected' : ''}>Orange+</option>
                                            </select>
                                        </div>
                                        ` : ''}
                                    </label>
                            ...
                            `;
                               }).join('') || '<div class="text-xs text-gray-500">Loading...</div>'}
                        </div>
                    </div>
                </div>
                
                <div class="text-xs text-yellow-600 bg-yellow-100 p-2 rounded">
                    💡 Selected cards provide stat bonuses and training multipliers. Choose level (1-40/45/50) and bond status. Orange bond enables friendship training bonuses.
                </div>
            </div>
        ...
        // Base stats reference with support card bonuses
        const baseStatsPanel = createElement('div', 'bg-gray-50 rounded-lg p-4');
        
        // Calculate support card bonuses if available
        const supportCardBonuses = window.Uma?.SupportCards?.calculateSupportCardStatBonuses ? 
            window.Uma.SupportCards.calculateSupportCardStatBonuses(
                (appState.selectedSupportCards || []).filter(cardData => cardData && cardData.card)
        ...
        baseStatsPanel.innerHTML = `
                ...
                ${Object.entries(character.baseStats).map(([stat, value]) => `
                    <div class="flex justify-between">
                        ...
                        <span class="font-medium text-gray-800">
                            ${value}
                            ${supportCardBonuses[stat] > 0 ? `<span class="text-green-600 ml-1">(+${supportCardBonuses[stat]})</span>` : ''}
                        </span>
                    </div>
                `).join('')}
            </div>
            <div class="mt-2 text-xs text-gray-500">
                ...
                ${Object.values(supportCardBonuses).some(v => v > 0) ? ' • Support card bonuses shown in green' : ''}
            </div>
        `;
        ...
        // Phase 4A Enhanced Character advice panel with archetype guidance and skill activation
        const advicePanel = createElement('div', 'bg-blue-50 rounded-lg p-4 space-y-4');
        
        // Phase 4A Enhancement: Skill Activation Feedback
        const skillActivationHTML = enhancedAdvice?.skillActivation ? `
            <div class="bg-indigo-50 border border-indigo-200 rounded-lg p-3">
                <h4 class="font-semibold text-indigo-800 mb-2 flex items-center gap-2">
                    <span>⚡</span>
                    Skill Activation Analysis
                </h4>
                <div class="text-sm text-indigo-800 bg-indigo-100 p-2 rounded">
                    ${enhancedAdvice.skillActivation.feedback}
                </div>
                <div class="text-xs text-indigo-600 mt-1">
                    Current activation rate: ${enhancedAdvice.skillActivation.rate}% (based on ${appState.stats.wit} Wit)
                </div>
            </div>
        ` : '';

        // Phase 4A Enhancement: Character Archetype Guidance
        const archetypeHTML = enhancedAdvice?.archetype ? `
        ...
        // Enhanced Training Priorities (mood-adjusted if available)
        const trainingPriorities = enhancedAdvice?.moodAdjustedPriorities || characterAdvice.priority;
        ...
        setTimeout(() => {
            ...
            // Phase 4A Enhancement: Mood selector
            const moodSelect = document.getElementById('mood-select');
            if (moodSelect) {
                moodSelect.addEventListener('change', (e) => {
                    const selectedMood = e.target.value;
                    updateAppState({ currentMood: selectedMood });
                    
                    // Update mood feedback
                    const moodFeedback = document.getElementById('mood-feedback');
                    if (moodFeedback && window.Uma?.Constants?.MOOD_MULTIPLIERS) {
                        const moodData = window.Uma.Constants.MOOD_MULTIPLIERS[selectedMood];
                        if (moodData) {
                            const efficiency = Math.round((moodData.value - 1) * 100);
                            const sign = efficiency > 0 ? '+' : '';
                            const message = efficiency === 0 ? 
                                '💡 Normal mood - baseline training efficiency' :
                                `💡 ${moodData.label} - ${sign}${efficiency}% training efficiency`;
                            moodFeedback.innerHTML = message;
                            moodFeedback.className = `text-sm p-2 rounded-md ${
                                efficiency > 0 ? 'bg-green-100 text-green-800' :
                                efficiency < 0 ? 'bg-red-100 text-red-800' :
            ...
            // Phase 4B Enhancement: Support card selection with levels and bonds
            const supportCardCheckboxes = document.querySelectorAll('.support-card-checkbox');
            supportCardCheckboxes.forEach(checkbox => {
                checkbox.addEventListener('change', (e) => {
                    const cardId = e.target.dataset.cardId;
                    const currentCards = [...(appState.selectedSupportCards || [])];
                    const isCurrentlySelected = currentCards.some(sc => sc.cardId === cardId);
                    
                    if (!isCurrentlySelected) {
                        // Add card if not already selected and under limit
                        if (currentCards.length < 6) {
                            const card = window.Uma?.SupportCards?.getSupportCardById(cardId);
                            currentCards.push({
                                cardId: cardId,
                                level: 1,
                                bondStatus: 'unbonded',
                                card: card // Store reference for calculations
                            });
                            updateAppState({ selectedSupportCards: currentCards });
                        } else {
                            // Uncheck if over limit
                            e.target.checked = false;
                            alert('You can only select up to 6 support cards');
                        }
                    } else {
                        // Remove card
                        const updatedCards = currentCards.filter(sc => sc.cardId !== cardId);
                        updateAppState({ selectedSupportCards: updatedCards });
                    }
                });
            });

            // Level and bond status change handlers
            const levelSelects = document.querySelectorAll('.support-card-level');
            levelSelects.forEach(select => {
                select.addEventListener('change', (e) => {
                    ...
                    const cardIndex = currentCards.findIndex(sc => sc.cardId === cardId);
                    if (cardIndex !== -1) {
                        currentCards[cardIndex].level = newLevel;
                        updateAppState({ selectedSupportCards: currentCards });
                    }
                });
            });

            const bondSelects = document.querySelectorAll('.support-card-bond');
            bondSelects.forEach(select => {
                select.addEventListener('change', (e) => {
                    const cardId = e.target.dataset.cardId;
                    const newBondStatus = e.target.value;
                    const currentCards = [...(appState.selectedSupportCards || [])];
                    
                    const cardIndex = currentCards.findIndex(sc => sc.cardId === cardId);
                    if (cardIndex !== -1) {
                        currentCards[cardIndex].bondStatus = newBondStatus;
                        updateAppState({ selectedSupportCards: currentCards });
                    }
                });
            });
