...
(function() {
    ...
    const CHARACTERS = [
      // 🆕 NEWEST CHARACTERS (July 2025) - Anime Collaboration
      {
        id: 'beyond_the_horizon_tokai_teio',
        name: '[Beyond the Horizon] Tokai Teio',
        rarity: 3,
        defaultStyle: 'pace-chaser',
        styles: ['pace-chaser'],
        aptitudes: { turf: 'A', dirt: 'G', sprint: 'F', mile: 'E', medium: 'A', long: 'B' },
        runningStyles: { front: 'D', pace: 'A', late: 'C', end: 'E' },
        statGrowth: { speed: 10, stamina: 10, power: 0, guts: 10, wit: 0 },
        baseStats: { speed: 89, stamina: 102, power: 80, guts: 86, wit: 93 },
        fanGoalThresholds: [5000],
        image: '🌅',
        uniqueSkill: 'Horizon\'s Edge',
        description: 'Anime collaboration variant with balanced stats and pace chaser specialization'
      },
      {
        id: 'end_of_skies_mejiro_mcqueen',
        name: '[End of Skies] Mejiro McQueen',
        rarity: 3,
        defaultStyle: 'pace-chaser',
        styles: ['pace-chaser', 'front-runner'],
        aptitudes: { turf: 'A', dirt: 'E', sprint: 'G', mile: 'F', medium: 'A', long: 'A' },
        runningStyles: { front: 'B', pace: 'A', late: 'D', end: 'F' },
        statGrowth: { speed: 0, stamina: 10, power: 10, guts: 0, wit: 10 },
        baseStats: { speed: 74, stamina: 110, power: 72, guts: 108, wit: 86 },
        fanGoalThresholds: [3000],
        image: '🌌',
        uniqueSkill: 'Celestial Dominance',
        description: 'Anime collaboration variant focusing on stamina and endurance racing'
      },
      {
        id: 'biwa_hayahide',
        name: 'Biwa Hayahide',
    ...
    // Phase 4A Enhancement: Validated Aptitude Multipliers (Conservative approach)
    // Based on Global community testing: S≈1.05×, A=1.0×, B≈0.9×
    const APTITUDE_MULTIPLIERS = {
      'S': 1.05,  // 5% bonus (validated by Global community testing)
      'A': 1.0,   // Baseline (confirmed across sources)
      'B': 0.9,   // 10% penalty (community consensus)
      'C': 0.85,  // 15% penalty (extrapolated)
      'D': 0.8,   // 20% penalty (extrapolated)
      'E': 0.75,  // 25% penalty (extrapolated)
      'F': 0.7,   // 30% penalty (extrapolated)
      'G': 0.6    // 40% penalty (extrapolated)
    };

    // Phase 4A Enhancement: Mood Multipliers for Training Efficiency
    // Based on validated community data for training stat gains (5 moods only)
    const MOOD_MULTIPLIERS = {
      'great': { value: 1.2, label: 'Great 😊', color: 'text-green-400' },
      'good': { value: 1.1, label: 'Good 🙂', color: 'text-blue-400' },
      'normal': { value: 1.0, label: 'Normal 😐', color: 'text-gray-400' },
      'bad': { value: 0.9, label: 'Bad 😟', color: 'text-orange-400' },
      'awful': { value: 0.8, label: 'Awful 😫', color: 'text-red-400' }
    };

    // Phase 4A Enhancement: Skill Activation Formula Constants  
    // Validated by multiple Global community sources
    const SKILL_ACTIVATION = {
      formula: 'max(100 - 9000 / wit, 20)',
      baseConstant: 9000,
      minimumRate: 20,
      // Helper function for calculating activation rate
      calculateRate: (witStat) => Math.max(100 - 9000 / witStat, 20)
    };

    // Phase 4A Enhancement: Character Archetype Classifications
    // Based on community consensus for training guidance (not rigid rules)
    const CHARACTER_ARCHETYPES = {
      'speed_specialist': {
        label: 'Speed Specialist',
        description: 'High Speed/Power, Front Runner style, short/mile distances',
        statPriorities: ['speed', 'power', 'wit'],
        preferredDistances: ['sprint', 'mile'],
        preferredStyles: ['front-runner', 'pace-chaser'],
        thresholds: { speed: 800, power: 600 }
      },
      'stamina_tank': {
        label: 'Stamina Tank', 
        description: 'High Stamina/Guts, End Closer style, long distances',
        statPriorities: ['stamina', 'guts', 'wit'],
        preferredDistances: ['medium', 'long'],
        preferredStyles: ['late-surger', 'end-closer'],
        thresholds: { stamina: 600, guts: 400 }
      },
      'balanced_runner': {
        label: 'Balanced Runner',
        description: 'Even stats, medium distances, versatile styles',
        statPriorities: ['speed', 'stamina', 'power'],
        preferredDistances: ['mile', 'medium'],
        preferredStyles: ['pace-chaser', 'late-surger'],
        thresholds: { speed: 650, stamina: 500, power: 500 }
      },
      'power_brawler': {
        label: 'Power Brawler',
        description: 'High Power for acceleration/positioning, medium specialists',
        statPriorities: ['power', 'speed', 'guts'],
        preferredDistances: ['mile', 'medium'],
        preferredStyles: ['pace-chaser', 'late-surger'],
        thresholds: { power: 600, speed: 600 }
      }
    };
